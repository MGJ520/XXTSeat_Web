...
def api_login():
    ...
    """
    data = request.json
    username_local = data.get('username_login_local')
    email_local = data.get('email_login_local')
    password_local = data.get('password_login_local')

    if email_local in users and check_password_hash(users[email_local], password_local):
        # 用户验证成功，创建JWT
        token = create_jwt(username_local, email_local)
...
def api_register():
    ...
    if email_local in users:
        # 用户已存在，返回错误信息
        return jsonify({'success': False, 'error': 'Email already exists'})
    else:
        # 注册新用户，存储密码哈希值
        users[email_local] = generate_password_hash(password_local)
...
@app.route('/api/get/contact', methods=['POST'])
@rate_limit_ip('/api/get/contact',HIGN_MAX_REQUESTS, HIGN_REQUEST_TIME_WINDOW)
def api_get_contact():
    db = AppointmentDB()
    # 尝试从JSON请求体中获取数据
    try:
